...
- 阿里云盘与迅雷云盘【待做】强制要求转存
- 集体使用时“指定一个用户会主动寻找上次使用的阿里账号，即子母号”设计减少转存次数降低风控，但是复用机制也有风险
- 在不同的分享share_id里，file_id只是这个分享内文件的唯一凭据。在多个分享中可能出现file_id一样但是文件不同
**策略**：及时删除转存后的所有内容（比如一天记为period）对于母号来言在一个period内只有初次解析时调用转存（发生在一个天赐子号用户身上），务必导出一份curl/gopeed任务保留sha1，crc64校验值。当然后端可选记录这些用于复盘
如果这个太麻烦了，那么个人下载完成后，请务必核对文件size是否一样，这样掉包概率可以再次下降
